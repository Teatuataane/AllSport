...
- **P3 — drop the repair archive tables once the ladders are settled.** `results_difficulty_archive_20260910025855` (52 rows) and `results_difficulty_preimage_20260910025855` (931 rows). Both verified locked down: HTTP 401 / `42501` through PostgREST. The pre-image is the ONLY way back for the ~85 re-encoded rows, so do not drop it while any ladder is still in question.

- **P3 — Golf labels say "(4 holes)" twice.** A repaired round reads `D4 Game (4 Holes) · 18 strokes (4 holes)`, because the gained-ladder pass appends the old `score_label` to the new rung name. Cosmetic, player-visible on /prs. One `regexp_replace` in a follow-up, or leave it.
