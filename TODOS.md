...
- **Archives dropped and the Golf label tidied** (2026-09-11, `20260911030444`). All three archive tables exported in full to `~/allsport-backups/difficulty-rebuild-20260911/` and verified (row counts matched, every id present and unique) BEFORE being dropped — the export is outside the repo on purpose, since the repo is public and those rows are player scores. `results_leg_extension_archive_20260801` went with them: its stated condition, "once the Leg Ext Hold call is settled", was met when Leg Ext Hold became `weight+time` in v0.7.1.0. Also fixed 23 Golf labels that read "strokes (4 holes)" twice, because the repair appended the old label to the new rung name.

- **The difficulty rebuild is applied to production** (2026-09-11), along with three migrations that were queued in front of it. Verified by querying the objects, never the ledger.
  - **Two of those three were recorded in CLAUDE.md as applied and verified, and had never been run.** `compute_event_placements` carried no `DISTINCT ON` and `set_session_date_from_started_at` did not exist. Live at that moment: **642 winning rows against 537 real player-wins**, **259 double-placed player-events**, and **26 of 64 sessions** with a `session_date` that disagreed with the NZ day of `started_at`. All now 537 / 0 / 0. An "applied and verified" line in a doc is not evidence; check `pg_proc`.
  - The repair **aborted on its own assertion** the first time (`10 rows on tiered events still carry no level`) and rolled back clean. Those ten were on events already tiered before the review, so no repair pass keyed on them and `doomed` could not see them. Fixed and re-applied.
  - Final: `results` 1388 → 1336, **52 archived**, rows carrying a level 454 → **879**, rows on a tiered event with no level **0**, taniwha budget breaches **0**, lifetime points unchanged.
...
`lib/leaderboardScores.ts` `GAME_RESULT_RUNG`, and check why Loco Chocko reads
