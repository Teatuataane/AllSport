# Grading Standards — Review

**Approved by Tāne on 16 September 2026 as a trial. These numbers run as they stand and are reviewed after the trial.** Edit this sheet and re-run `node scripts/apply-standards-sheet.mjs` to change any of them.

Every standard for all 120 events: one line per colour. Go through them the way you went through the difficulty sheet.

## How to fill this in

Each event lists its colours from Kiwikiwi up. **Edit the value after the colour.** That is the whole job.

- **Change a standard:** type over the value.
- **Split men and women:** replace `**Everyone**` with a `**Men**` list and a `**Women**` list.
- **Leave a note:** write it on its own line starting with `>`, and I will pick it up.

How a value is written:

- `D3 · 20` — twenty reps on level D3. `D3 · 30s` or `D3 · 1:30` — a hold that long on D3.
- `D3 · ≤ 4:05` — D3 done in 4:05 or faster. `D3 · finish` — D3 done, any time.
- `D6 · +10kg` — the weighted top level with 10kg added. `D2 · 15m` — a 15m throw on D2; `D1 · any` — any throw.
- `1.1× BW` — a lift of 1.1 times the middle of your bodyweight band. `empty bar` — any lift.
- `64cm`, `1:30`, `12kg · 30s` — a jump, a hold, a load held for that long.

Three rules:

1. **Each colour must be harder than the one before.** A higher level always beats a lower one, however few reps.
2. **Write the Open standard only.** Juniors, Masters and Grandmasters are shifted up the ladder automatically.
3. **Game events list six colours.** Poroporo and above come from the head-to-head rating, after ten games.

Every event says where its numbers came from. **Approved** means you settled it in review. **Drafted** means I wrote it,
with the reason underneath: those are the ones to read hardest, and the hand-drafted ones hardest of all.

*generated 2026-09-15 · nothing above this line needs editing*

---

## 1. Maximal Strength

### 1. 1A Press

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 8 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: one arm, about a third of a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.12× BW
- Karaka: 0.15× BW
- Kōwhai: 0.18× BW
- Kākāriki: 0.21× BW
- Kahurangi: 0.25× BW
- Poroporo: 0.28× BW
- Parahi: 0.31× BW
- Hiriwa: 0.33× BW
- Kōura: 0.36× BW
- Uenuku: 0.41× BW
- Taniwha: 0.5× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.08× BW
- Karaka: 0.1× BW
- Kōwhai: 0.12× BW
- Kākāriki: 0.14× BW
- Kahurangi: 0.17× BW
- Poroporo: 0.18× BW
- Parahi: 0.2× BW
- Hiriwa: 0.22× BW
- Kōura: 0.25× BW
- Uenuku: 0.28× BW
- Taniwha: 0.33× BW

### 2. Deadlift

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 3 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: about 1.6 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.56× BW
- Karaka: 0.72× BW
- Kōwhai: 0.88× BW
- Kākāriki: 1.04× BW
- Kahurangi: 1.2× BW
- Poroporo: 1.36× BW
- Parahi: 1.49× BW
- Hiriwa: 1.6× BW
- Kōura: 1.76× BW
- Uenuku: 2× BW
- Taniwha: 2.4× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.37× BW
- Karaka: 0.48× BW
- Kōwhai: 0.58× BW
- Kākāriki: 0.69× BW
- Kahurangi: 0.8× BW
- Poroporo: 0.9× BW
- Parahi: 0.99× BW
- Hiriwa: 1.09× BW
- Kōura: 1.2× BW
- Uenuku: 1.36× BW
- Taniwha: 1.6× BW

### 3. Clean & Press

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 5 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: a strict overhead press, about 0.65 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.23× BW
- Karaka: 0.29× BW
- Kōwhai: 0.36× BW
- Kākāriki: 0.42× BW
- Kahurangi: 0.49× BW
- Poroporo: 0.55× BW
- Parahi: 0.6× BW
- Hiriwa: 0.65× BW
- Kōura: 0.72× BW
- Uenuku: 0.81× BW
- Taniwha: 0.98× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.15× BW
- Karaka: 0.2× BW
- Kōwhai: 0.23× BW
- Kākāriki: 0.28× BW
- Kahurangi: 0.33× BW
- Poroporo: 0.36× BW
- Parahi: 0.4× BW
- Hiriwa: 0.44× BW
- Kōura: 0.49× BW
- Uenuku: 0.55× BW
- Taniwha: 0.65× BW

### 4. Pause Dips

*Scored by: difficulty level, then most reps · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15
- Poroporo: D4 · 5
- Parahi: D4 · 15
- Hiriwa: D5 · 5
- Kōura: D5 · 15
- Uenuku: D6 · +10kg
- Taniwha: D6 · +25kg

### 5. Pause Chinup

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15
- Poroporo: D4 · 5
- Parahi: D4 · 15
- Hiriwa: D5 · 5
- Kōura: D5 · 15
- Uenuku: D6 · +10kg
- Taniwha: D6 · +20kg

### 6. Pause Back Squat

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 5 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: about 1.3 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.45× BW
- Karaka: 0.59× BW
- Kōwhai: 0.72× BW
- Kākāriki: 0.85× BW
- Kahurangi: 0.98× BW
- Poroporo: 1.11× BW
- Parahi: 1.21× BW
- Hiriwa: 1.3× BW
- Kōura: 1.43× BW
- Uenuku: 1.63× BW
- Taniwha: 1.95× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.3× BW
- Karaka: 0.39× BW
- Kōwhai: 0.47× BW
- Kākāriki: 0.56× BW
- Kahurangi: 0.65× BW
- Poroporo: 0.73× BW
- Parahi: 0.81× BW
- Hiriwa: 0.88× BW
- Kōura: 0.98× BW
- Uenuku: 1.11× BW
- Taniwha: 1.3× BW

### 7. Zercher Dead

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 5 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: about 1.2 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.42× BW
- Karaka: 0.54× BW
- Kōwhai: 0.66× BW
- Kākāriki: 0.78× BW
- Kahurangi: 0.9× BW
- Poroporo: 1.02× BW
- Parahi: 1.12× BW
- Hiriwa: 1.2× BW
- Kōura: 1.32× BW
- Uenuku: 1.5× BW
- Taniwha: 1.8× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.28× BW
- Karaka: 0.36× BW
- Kōwhai: 0.43× BW
- Kākāriki: 0.52× BW
- Kahurangi: 0.6× BW
- Poroporo: 0.67× BW
- Parahi: 0.74× BW
- Hiriwa: 0.82× BW
- Kōura: 0.9× BW
- Uenuku: 1.02× BW
- Taniwha: 1.2× BW

### 8. Pause Bench

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 14 players have scored it*

> Approved in review.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.35× BW
- Karaka: 0.45× BW
- Kōwhai: 0.55× BW
- Kākāriki: 0.65× BW
- Kahurangi: 0.75× BW
- Poroporo: 0.85× BW
- Parahi: 0.93× BW
- Hiriwa: 1× BW
- Kōura: 1.1× BW
- Uenuku: 1.25× BW
- Taniwha: 1.5× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.23× BW
- Karaka: 0.3× BW
- Kōwhai: 0.36× BW
- Kākāriki: 0.43× BW
- Kahurangi: 0.5× BW
- Poroporo: 0.56× BW
- Parahi: 0.62× BW
- Hiriwa: 0.68× BW
- Kōura: 0.75× BW
- Uenuku: 0.85× BW
- Taniwha: 1× BW

### 9. Turkish Getup

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 3 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: one arm, about a third of a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.12× BW
- Karaka: 0.15× BW
- Kōwhai: 0.18× BW
- Kākāriki: 0.21× BW
- Kahurangi: 0.25× BW
- Poroporo: 0.28× BW
- Parahi: 0.31× BW
- Hiriwa: 0.33× BW
- Kōura: 0.36× BW
- Uenuku: 0.41× BW
- Taniwha: 0.5× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.08× BW
- Karaka: 0.1× BW
- Kōwhai: 0.12× BW
- Kākāriki: 0.14× BW
- Kahurangi: 0.17× BW
- Poroporo: 0.18× BW
- Parahi: 0.2× BW
- Hiriwa: 0.22× BW
- Kōura: 0.25× BW
- Uenuku: 0.28× BW
- Taniwha: 0.33× BW

### 10. Arthur Lift

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 2 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: a clean and jerk to behind the neck, about 0.7 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.24× BW
- Karaka: 0.32× BW
- Kōwhai: 0.39× BW
- Kākāriki: 0.45× BW
- Kahurangi: 0.52× BW
- Poroporo: 0.6× BW
- Parahi: 0.65× BW
- Hiriwa: 0.7× BW
- Kōura: 0.77× BW
- Uenuku: 0.88× BW
- Taniwha: 1.05× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.16× BW
- Karaka: 0.21× BW
- Kōwhai: 0.25× BW
- Kākāriki: 0.3× BW
- Kahurangi: 0.35× BW
- Poroporo: 0.39× BW
- Parahi: 0.43× BW
- Hiriwa: 0.48× BW
- Kōura: 0.52× BW
- Uenuku: 0.6× BW
- Taniwha: 0.7× BW

### 11. Pause Row

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 0 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: about 0.9 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.32× BW
- Karaka: 0.41× BW
- Kōwhai: 0.5× BW
- Kākāriki: 0.59× BW
- Kahurangi: 0.68× BW
- Poroporo: 0.77× BW
- Parahi: 0.84× BW
- Hiriwa: 0.9× BW
- Kōura: 0.99× BW
- Uenuku: 1.13× BW
- Taniwha: 1.35× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.21× BW
- Karaka: 0.27× BW
- Kōwhai: 0.32× BW
- Kākāriki: 0.39× BW
- Kahurangi: 0.45× BW
- Poroporo: 0.5× BW
- Parahi: 0.56× BW
- Hiriwa: 0.61× BW
- Kōura: 0.68× BW
- Uenuku: 0.77× BW
- Taniwha: 0.9× BW

### 12. Pause Front Squat

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 0 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: about 1.1 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.39× BW
- Karaka: 0.5× BW
- Kōwhai: 0.61× BW
- Kākāriki: 0.72× BW
- Kahurangi: 0.83× BW
- Poroporo: 0.94× BW
- Parahi: 1.02× BW
- Hiriwa: 1.1× BW
- Kōura: 1.21× BW
- Uenuku: 1.38× BW
- Taniwha: 1.65× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.25× BW
- Karaka: 0.33× BW
- Kōwhai: 0.4× BW
- Kākāriki: 0.47× BW
- Kahurangi: 0.55× BW
- Poroporo: 0.62× BW
- Parahi: 0.68× BW
- Hiriwa: 0.75× BW
- Kōura: 0.83× BW
- Uenuku: 0.94× BW
- Taniwha: 1.1× BW

---

## 2. Calisthenics

### 13. 1 Leg Squat

*Scored by: difficulty level, then most reps · 14 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15
- Poroporo: D4 · 5
- Parahi: D4 · 15
- Hiriwa: D5 · 5
- Kōura: D5 · 15
- Uenuku: D6 · 5
- Taniwha: D6 · 15

### 14. Human Flag

*Scored by: difficulty level, then longest hold · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 10s
- Taniwha: D6 · 30s

### 15. Windshield Wipers

*Scored by: difficulty level, then most reps · 5 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 10
- Karaka: D1 · 20
- Kōwhai: D2 · 5
- Kākāriki: D2 · 10
- Kahurangi: D2 · 20
- Poroporo: D3 · 5
- Parahi: D3 · 15
- Hiriwa: D4 · 5
- Kōura: D4 · 15
- Uenuku: D5 · 5
- Taniwha: D5 · 15

### 16. Planche

*Scored by: difficulty level, then longest hold · 5 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 30s
- Taniwha: D7 · 30s

### 17. Back Lever

*Scored by: difficulty level, then longest hold · 4 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 30s
- Taniwha: D7 · 30s

### 18. Iron Cross

*Scored by: difficulty level, then longest hold · 4 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 30s
- Taniwha: D7 · 30s

### 19. Front Lever

*Scored by: difficulty level, then longest hold · 4 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 30s
- Taniwha: D7 · 30s

### 20. Chin Hang

*Scored by: difficulty level, then longest hold · 6 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 10s
- Taniwha: D6 · 30s

### 21. Climbing

*Scored by: difficulty level, then fastest time · 2 players have scored it*

> Drafted by hand. The four hardest rungs are one colour each for completing them. NOTE: the lowest three rungs are HANGS on a ladder ranked fastest-first, so a longer hang scores worse. That came from the difficulty review; say if the hangs should move off this ladder.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 20s
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 20s
- Kākāriki: D3 · finish
- Kahurangi: D3 · ≤ 20s
- Poroporo: D4 · finish
- Parahi: D4 · ≤ 20s
- Hiriwa: D5 · finish
- Kōura: D6 · finish
- Uenuku: D7 · finish
- Taniwha: D8 · finish

### 22. Handstand

*Scored by: difficulty level, then longest hold · 2 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 20s
- Karaka: D1 · 45s
- Kōwhai: D2 · 10s
- Kākāriki: D2 · 20s
- Kahurangi: D2 · 45s
- Poroporo: D3 · 10s
- Parahi: D3 · 30s
- Hiriwa: D4 · 10s
- Kōura: D4 · 30s
- Uenuku: D5 · 10s
- Taniwha: D5 · 30s

### 23. Headstand

*Scored by: difficulty level, then longest hold · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 10s
- Taniwha: D6 · 30s

### 24. L-Sit Hold

*Scored by: difficulty level, then longest hold · 4 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 10s
- Taniwha: D6 · 30s

---

## 3. Power

### 25. Kelly Snatch

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 4 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: one arm, about 0.4 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.14× BW
- Karaka: 0.18× BW
- Kōwhai: 0.22× BW
- Kākāriki: 0.26× BW
- Kahurangi: 0.3× BW
- Poroporo: 0.34× BW
- Parahi: 0.37× BW
- Hiriwa: 0.4× BW
- Kōura: 0.44× BW
- Uenuku: 0.5× BW
- Taniwha: 0.6× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.09× BW
- Karaka: 0.12× BW
- Kōwhai: 0.14× BW
- Kākāriki: 0.17× BW
- Kahurangi: 0.2× BW
- Poroporo: 0.22× BW
- Parahi: 0.25× BW
- Hiriwa: 0.27× BW
- Kōura: 0.3× BW
- Uenuku: 0.34× BW
- Taniwha: 0.4× BW

### 26. 1A Snatch

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 4 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: one arm, about 0.4 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.14× BW
- Karaka: 0.18× BW
- Kōwhai: 0.22× BW
- Kākāriki: 0.26× BW
- Kahurangi: 0.3× BW
- Poroporo: 0.34× BW
- Parahi: 0.37× BW
- Hiriwa: 0.4× BW
- Kōura: 0.44× BW
- Uenuku: 0.5× BW
- Taniwha: 0.6× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.09× BW
- Karaka: 0.12× BW
- Kōwhai: 0.14× BW
- Kākāriki: 0.17× BW
- Kahurangi: 0.2× BW
- Poroporo: 0.22× BW
- Parahi: 0.25× BW
- Hiriwa: 0.27× BW
- Kōura: 0.3× BW
- Uenuku: 0.34× BW
- Taniwha: 0.4× BW

### 27. Javelin

*Scored by: difficulty level, then furthest throw · 4 players have scored it*

> Drafted by hand. Four colours on each implement. A throw with a heavier implement always outranks a lighter one, as on every ladder.

**Men**

- Kiwikiwi: D1 · any
- Whero: D1 · 10m
- Karaka: D1 · 15m
- Kōwhai: D1 · 20m
- Kākāriki: D2 · 10m
- Kahurangi: D2 · 15m
- Poroporo: D2 · 20m
- Parahi: D2 · 25m
- Hiriwa: D3 · 15m
- Kōura: D3 · 25m
- Uenuku: D3 · 35m
- Taniwha: D3 · 45m

**Women**

- Kiwikiwi: D1 · any
- Whero: D1 · 8m
- Karaka: D1 · 12m
- Kōwhai: D1 · 16m
- Kākāriki: D2 · 8m
- Kahurangi: D2 · 12m
- Poroporo: D2 · 16m
- Parahi: D2 · 20m
- Hiriwa: D3 · 10m
- Kōura: D3 · 17m
- Uenuku: D3 · 24m
- Taniwha: D3 · 32m

### 28. Shotput

*Scored by: difficulty level, then furthest throw · 5 players have scored it*

> Drafted by hand. Four colours on each implement. The tennis-ball rung is thrown, so its distances are far longer.

**Men**

- Kiwikiwi: D1 · any
- Whero: D1 · 15m
- Karaka: D1 · 25m
- Kōwhai: D1 · 35m
- Kākāriki: D2 · 4m
- Kahurangi: D2 · 6m
- Poroporo: D2 · 8m
- Parahi: D2 · 10m
- Hiriwa: D3 · 5m
- Kōura: D3 · 7m
- Uenuku: D3 · 9m
- Taniwha: D3 · 11m

**Women**

- Kiwikiwi: D1 · any
- Whero: D1 · 10m
- Karaka: D1 · 18m
- Kōwhai: D1 · 25m
- Kākāriki: D2 · 3m
- Kahurangi: D2 · 5m
- Poroporo: D2 · 7m
- Parahi: D2 · 9m
- Hiriwa: D3 · 4m
- Kōura: D3 · 6m
- Uenuku: D3 · 8m
- Taniwha: D3 · 10m

### 29. Australian Football

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 30. Vertical Jump

*Scored by: furthest or highest · 14 players have scored it*

> Approved in review.

**Men**

- Kiwikiwi: 10cm
- Whero: 20cm
- Karaka: 27cm
- Kōwhai: 33cm
- Kākāriki: 39cm
- Kahurangi: 45cm
- Poroporo: 51cm
- Parahi: 54cm
- Hiriwa: 57cm
- Kōura: 60cm
- Uenuku: 62cm
- Taniwha: 64cm

**Women**

- Kiwikiwi: 8cm
- Whero: 15cm
- Karaka: 20cm
- Kōwhai: 25cm
- Kākāriki: 29cm
- Kahurangi: 33cm
- Poroporo: 38cm
- Parahi: 40cm
- Hiriwa: 42cm
- Kōura: 44cm
- Uenuku: 47cm
- Taniwha: 50cm

### 31. Clean & Jerk

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 6 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: about a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.35× BW
- Karaka: 0.45× BW
- Kōwhai: 0.55× BW
- Kākāriki: 0.65× BW
- Kahurangi: 0.75× BW
- Poroporo: 0.85× BW
- Parahi: 0.93× BW
- Hiriwa: 1× BW
- Kōura: 1.1× BW
- Uenuku: 1.25× BW
- Taniwha: 1.5× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.23× BW
- Karaka: 0.3× BW
- Kōwhai: 0.36× BW
- Kākāriki: 0.43× BW
- Kahurangi: 0.5× BW
- Poroporo: 0.56× BW
- Parahi: 0.62× BW
- Hiriwa: 0.68× BW
- Kōura: 0.75× BW
- Uenuku: 0.85× BW
- Taniwha: 1× BW

### 32. Snatch

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 3 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: about 0.8 times a bench press.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.28× BW
- Karaka: 0.36× BW
- Kōwhai: 0.44× BW
- Kākāriki: 0.52× BW
- Kahurangi: 0.6× BW
- Poroporo: 0.68× BW
- Parahi: 0.74× BW
- Hiriwa: 0.8× BW
- Kōura: 0.88× BW
- Uenuku: 1× BW
- Taniwha: 1.2× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.18× BW
- Karaka: 0.24× BW
- Kōwhai: 0.29× BW
- Kākāriki: 0.34× BW
- Kahurangi: 0.4× BW
- Poroporo: 0.45× BW
- Parahi: 0.5× BW
- Hiriwa: 0.54× BW
- Kōura: 0.6× BW
- Uenuku: 0.68× BW
- Taniwha: 0.8× BW

### 33. Standing Broad Jump

*Scored by: furthest or highest · 2 players have scored it*

> Drafted by hand. Built like the approved Vertical Jump: the median adult on Kahurangi, a steep climb to the top.

**Men**

- Kiwikiwi: 100cm
- Whero: 130cm
- Karaka: 150cm
- Kōwhai: 165cm
- Kākāriki: 180cm
- Kahurangi: 195cm
- Poroporo: 210cm
- Parahi: 225cm
- Hiriwa: 240cm
- Kōura: 255cm
- Uenuku: 270cm
- Taniwha: 290cm

**Women**

- Kiwikiwi: 80cm
- Whero: 105cm
- Karaka: 120cm
- Kōwhai: 135cm
- Kākāriki: 145cm
- Kahurangi: 155cm
- Poroporo: 165cm
- Parahi: 175cm
- Hiriwa: 185cm
- Kōura: 195cm
- Uenuku: 210cm
- Taniwha: 230cm

### 34. High Jump

*Scored by: furthest or highest · 2 players have scored it*

> Drafted by hand. Bar height cleared, any technique.

**Men**

- Kiwikiwi: 60cm
- Whero: 80cm
- Karaka: 95cm
- Kōwhai: 105cm
- Kākāriki: 115cm
- Kahurangi: 125cm
- Poroporo: 135cm
- Parahi: 145cm
- Hiriwa: 155cm
- Kōura: 165cm
- Uenuku: 175cm
- Taniwha: 190cm

**Women**

- Kiwikiwi: 50cm
- Whero: 65cm
- Karaka: 75cm
- Kōwhai: 85cm
- Kākāriki: 95cm
- Kahurangi: 100cm
- Poroporo: 110cm
- Parahi: 115cm
- Hiriwa: 125cm
- Kōura: 135cm
- Uenuku: 145cm
- Taniwha: 160cm

### 35. Arm Wrestling

*Scored by: difficulty level, then longest hold · 2 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 30s
- Kahurangi: D4 · 30s

*Poroporo and above: the head-to-head rating.*

### 36. Tug of War

*Scored by: difficulty level, then fastest time · 0 players have scored it*

> Drafted by hand. A 10m hand-over-hand sled pull at each load.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 20s
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 25s
- Kākāriki: D3 · ≤ 40s
- Kahurangi: D4 · ≤ 1:00

*Poroporo and above: the head-to-head rating.*

---

## 4. Speed

### 37. 100m Sprint

*Scored by: difficulty level, then fastest time · 11 players have scored it*

> Drafted by hand. Three colours walking, three sprinting. The median adult man sprints 100m in about 15.5 seconds.

**Men**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:15
- Karaka: D1 · ≤ 1:00
- Kōwhai: D2 · ≤ 19s
- Kākāriki: D2 · ≤ 17s
- Kahurangi: D2 · ≤ 15.5s

**Women**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:15
- Karaka: D1 · ≤ 1:00
- Kōwhai: D2 · ≤ 22s
- Kākāriki: D2 · ≤ 19.5s
- Kahurangi: D2 · ≤ 17.5s

*Poroporo and above: the head-to-head rating.*

### 38. Tag

*Scored by: difficulty level, then fastest time · 4 players have scored it*

> Drafted by hand. Six calls in a 10m grid. A new drill, so these times are the roughest on the sheet.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 40s
- Karaka: D1 · ≤ 34s
- Kōwhai: D1 · ≤ 29s
- Kākāriki: D1 · ≤ 25s
- Kahurangi: D1 · ≤ 22s

*Poroporo and above: the head-to-head rating.*

### 39. T-Race

*Scored by: difficulty level, then fastest time · 4 players have scored it*

> Drafted by hand. The T agility test: the median adult runs it in about 11 to 12 seconds.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 25s
- Karaka: D1 · ≤ 20s
- Kōwhai: D2 · ≤ 14s
- Kākāriki: D2 · ≤ 12.5s
- Kahurangi: D2 · ≤ 11.5s

*Poroporo and above: the head-to-head rating.*

### 40. Beach Flags

*Scored by: difficulty level, then fastest time · 7 players have scored it*

> Drafted by hand. From face down to a flag 15m away.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 5.5s
- Karaka: D1 · ≤ 4.8s
- Kōwhai: D1 · ≤ 4.3s
- Kākāriki: D1 · ≤ 3.9s
- Kahurangi: D1 · ≤ 3.6s

*Poroporo and above: the head-to-head rating.*

### 41. 200m Sprint

*Scored by: difficulty level, then fastest time · 3 players have scored it*

> Drafted by hand. As the 100m.

**Men**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 2:30
- Karaka: D1 · ≤ 2:05
- Kōwhai: D2 · ≤ 40s
- Kākāriki: D2 · ≤ 35s
- Kahurangi: D2 · ≤ 32s

**Women**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 2:30
- Karaka: D1 · ≤ 2:05
- Kōwhai: D2 · ≤ 46s
- Kākāriki: D2 · ≤ 40s
- Kahurangi: D2 · ≤ 37s

*Poroporo and above: the head-to-head rating.*

### 42. Touch Rugby

*Scored by: difficulty level, then most reps · 5 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 43. Repeat High Jump

*Scored by: difficulty level, then fastest time · 5 players have scored it*

> Drafted by hand. Two colours a height: complete the set, then complete it inside 30 seconds. The number of jumps is set by the kaiwhakawā, so the time assumes it never changes.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 30s
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 30s
- Kākāriki: D3 · finish
- Kahurangi: D3 · ≤ 30s
- Poroporo: D4 · finish
- Parahi: D4 · ≤ 30s
- Hiriwa: D5 · finish
- Kōura: D5 · ≤ 30s
- Uenuku: D6 · finish
- Taniwha: D6 · ≤ 30s

### 44. Rats & Rabbits

*Scored by: difficulty level, then fastest time · 3 players have scored it*

> Drafted by hand. Timed from the call to the line, 10m away. A new drill.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 3.5s
- Karaka: D1 · ≤ 3s
- Kōwhai: D1 · ≤ 2.7s
- Kākāriki: D1 · ≤ 2.5s
- Kahurangi: D1 · ≤ 2.3s

*Poroporo and above: the head-to-head rating.*

### 45. Speed Chess

*Scored by: difficulty level, then most reps · 3 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 3
- Whero: D1 · 8
- Karaka: D2 · 3
- Kōwhai: D2 · 8
- Kākāriki: D3 · 3
- Kahurangi: D3 · 8

*Poroporo and above: the head-to-head rating.*

### 46. American Football

*Scored by: difficulty level, then most reps · 0 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 47. Capture the Flag

*Scored by: difficulty level, then fastest time · 0 players have scored it*

> Drafted by hand. About 30m with a pickup. A new drill.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 10s
- Karaka: D1 · ≤ 8.5s
- Kōwhai: D1 · ≤ 7.5s
- Kākāriki: D1 · ≤ 6.8s
- Kahurangi: D1 · ≤ 6.2s

*Poroporo and above: the head-to-head rating.*

### 48. Kabaddi

*Scored by: difficulty level, then most reps · 2 players have scored it*

> Drafted by hand. Five cones make five colours, so the drill stops at Kākāriki and Kahurangi comes only from the rating. Add cones to the drill if the drill should reach Kahurangi.

**Everyone**

- Kiwikiwi: D1 · 1
- Whero: D1 · 2
- Karaka: D1 · 3
- Kōwhai: D1 · 4
- Kākāriki: D1 · 5

*Poroporo and above: the head-to-head rating.*

---

## 5. Anaerobic Endurance

### 49. Chinup Contest

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15
- Poroporo: D4 · 5
- Parahi: D4 · 15
- Hiriwa: D5 · 5
- Kōura: D5 · 15
- Uenuku: D6 · 5
- Taniwha: D6 · 15

### 50. Pushup Contest

*Scored by: difficulty level, then most reps · 13 players have scored it*

> Approved in review.

**Men**

- Kiwikiwi: D1 · 10
- Whero: D1 · 20
- Karaka: D2 · 10
- Kōwhai: D2 · 20
- Kākāriki: D2 · 30
- Kahurangi: D3 · 10
- Poroporo: D3 · 20
- Parahi: D3 · 30
- Hiriwa: D4 · 5
- Kōura: D4 · 20
- Uenuku: D5 · 10
- Taniwha: D6 · 10

**Women**

- Kiwikiwi: D1 · 10
- Whero: D1 · 20
- Karaka: D2 · 10
- Kōwhai: D2 · 20
- Kākāriki: D2 · 30
- Kahurangi: D3 · 6
- Poroporo: D3 · 12
- Parahi: D3 · 18
- Hiriwa: D4 · 3
- Kōura: D4 · 10
- Uenuku: D5 · 5
- Taniwha: D6 · 5

### 51. Tibialis Curl

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 5 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: a small muscle: a fifth of a bench press. A ratio of bodyweight sits oddly on a lift this size.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.07× BW
- Karaka: 0.09× BW
- Kōwhai: 0.11× BW
- Kākāriki: 0.13× BW
- Kahurangi: 0.15× BW
- Poroporo: 0.17× BW
- Parahi: 0.19× BW
- Hiriwa: 0.2× BW
- Kōura: 0.22× BW
- Uenuku: 0.25× BW
- Taniwha: 0.3× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.05× BW
- Karaka: 0.06× BW
- Kōwhai: 0.07× BW
- Kākāriki: 0.09× BW
- Kahurangi: 0.1× BW
- Poroporo: 0.11× BW
- Parahi: 0.12× BW
- Hiriwa: 0.14× BW
- Kōura: 0.15× BW
- Uenuku: 0.17× BW
- Taniwha: 0.2× BW

### 52. Finger Pushup

*Scored by: difficulty level, then most reps · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15
- Poroporo: D4 · 5
- Parahi: D4 · 15
- Hiriwa: D5 · 5
- Kōura: D5 · 15
- Uenuku: D6 · 10
- Taniwha: D7 · 10

### 53. GHD Situp

*Scored by: difficulty level, then most reps · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 10
- Karaka: D1 · 20
- Kōwhai: D2 · 5
- Kākāriki: D2 · 10
- Kahurangi: D2 · 20
- Poroporo: D3 · 5
- Parahi: D3 · 15
- Hiriwa: D4 · 5
- Kōura: D4 · 15
- Uenuku: D5 · +10kg
- Taniwha: D5 · +20kg

### 54. Leg Ext Hold

*Scored by: heaviest load, then longest hold · 1 players have scored it*

> Drafted by hand. A 30-second hold at each load from Parahi down, after three bodyweight holds. The loads follow the old ladder, extended past 24kg.

**Everyone**

- Kiwikiwi: BW · 15s
- Whero: BW · 30s
- Karaka: BW · 60s
- Kōwhai: 2kg · 30s
- Kākāriki: 4kg · 30s
- Kahurangi: 8kg · 30s
- Poroporo: 12kg · 30s
- Parahi: 16kg · 30s
- Hiriwa: 20kg · 30s
- Kōura: 24kg · 30s
- Uenuku: 28kg · 30s
- Taniwha: 32kg · 30s

### 55. Ab Rollout

*Scored by: difficulty level, then most reps · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 10
- Karaka: D1 · 20
- Kōwhai: D2 · 5
- Kākāriki: D2 · 10
- Kahurangi: D2 · 20
- Poroporo: D3 · 5
- Parahi: D3 · 15
- Hiriwa: D4 · 5
- Kōura: D4 · 15
- Uenuku: D5 · 5
- Taniwha: D5 · 15

### 56. Hamstring Curl

*Scored by: difficulty level, then most reps · 2 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 10
- Karaka: D1 · 20
- Kōwhai: D2 · 5
- Kākāriki: D2 · 10
- Kahurangi: D2 · 20
- Poroporo: D3 · 5
- Parahi: D3 · 15
- Hiriwa: D4 · 5
- Kōura: D4 · 15
- Uenuku: D5 · 5
- Taniwha: D5 · 15

### 57. Sandbag to Shoulder

*Scored by: difficulty level, then most reps · 2 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15
- Poroporo: D4 · 5
- Parahi: D4 · 15
- Hiriwa: D5 · 5
- Kōura: D5 · 15
- Uenuku: D6 · 5
- Taniwha: D6 · 15

### 58. Wall Sit

*Scored by: longest hold · 2 players have scored it*

> Drafted by hand. The median adult holds about a minute and a half.

**Everyone**

- Kiwikiwi: 10s
- Whero: 20s
- Karaka: 30s
- Kōwhai: 45s
- Kākāriki: 1:00
- Kahurangi: 1:30
- Poroporo: 2:00
- Parahi: 2:30
- Hiriwa: 3:00
- Kōura: 4:00
- Uenuku: 5:00
- Taniwha: 7:00

### 59. Toe Lift

*Scored by: heaviest weight lifted, as a ratio of bodyweight · 2 players have scored it*

> Drafted by hand. The approved bench ratios scaled for this lift: a small muscle: a fifth of a bench press. The event itself was flagged as uncertain in July.

**Men**

- Kiwikiwi: empty bar
- Whero: 0.07× BW
- Karaka: 0.09× BW
- Kōwhai: 0.11× BW
- Kākāriki: 0.13× BW
- Kahurangi: 0.15× BW
- Poroporo: 0.17× BW
- Parahi: 0.19× BW
- Hiriwa: 0.2× BW
- Kōura: 0.22× BW
- Uenuku: 0.25× BW
- Taniwha: 0.3× BW

**Women**

- Kiwikiwi: empty bar
- Whero: 0.05× BW
- Karaka: 0.06× BW
- Kōwhai: 0.07× BW
- Kākāriki: 0.09× BW
- Kahurangi: 0.1× BW
- Poroporo: 0.11× BW
- Parahi: 0.12× BW
- Hiriwa: 0.14× BW
- Kōura: 0.15× BW
- Uenuku: 0.17× BW
- Taniwha: 0.2× BW

### 60. Lunges

*Scored by: difficulty level, then most reps · 0 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 10
- Karaka: D1 · 20
- Kōwhai: D2 · 5
- Kākāriki: D2 · 10
- Kahurangi: D2 · 20
- Poroporo: D3 · 5
- Parahi: D3 · 15
- Hiriwa: D4 · 5
- Kōura: D4 · 15
- Uenuku: D5 · 5
- Taniwha: D5 · 15

---

## 6. Aerobic Endurance

### 61. Burpee Broad Jump

*Scored by: difficulty level, then fastest time · 3 players have scored it*

> Drafted by hand. About 55 burpee broad jumps to 100m. Taniwha is 200m at under five seconds a rep.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:30
- Karaka: D1 · ≤ 1:10
- Kōwhai: D2 · ≤ 3:00
- Kākāriki: D2 · ≤ 2:30
- Kahurangi: D2 · ≤ 2:10
- Poroporo: D3 · ≤ 6:00
- Parahi: D3 · ≤ 5:00
- Hiriwa: D3 · ≤ 4:20
- Kōura: D4 · ≤ 12:00
- Uenuku: D4 · ≤ 10:00
- Taniwha: D4 · ≤ 8:30

### 62. Running

*Scored by: difficulty level, then fastest time · 3 players have scored it*

> Drafted by hand. Anyone can finish 1000m, so finishing cannot be a high colour: most colours are times on the longest rung. Women's times are the men's plus 12%.

**Men**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:10
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 2:30
- Kākāriki: D2 · ≤ 2:05
- Kahurangi: D3 · ≤ 6:00
- Poroporo: D3 · ≤ 5:00
- Parahi: D3 · ≤ 4:30
- Hiriwa: D3 · ≤ 4:05
- Kōura: D3 · ≤ 3:45
- Uenuku: D3 · ≤ 3:30
- Taniwha: D3 · ≤ 3:15

**Women**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:20
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 2:50
- Kākāriki: D2 · ≤ 2:20
- Kahurangi: D3 · ≤ 6:45
- Poroporo: D3 · ≤ 5:35
- Parahi: D3 · ≤ 5:00
- Hiriwa: D3 · ≤ 4:35
- Kōura: D3 · ≤ 4:10
- Uenuku: D3 · ≤ 3:55
- Taniwha: D3 · ≤ 3:40

### 63. Cycling

*Scored by: difficulty level, then fastest time · 4 players have scored it*

> Drafted by hand. As Running. These assume a stationary bike; the bike used changes every time.

**Men**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 0:35
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 1:05
- Kākāriki: D2 · ≤ 0:55
- Kahurangi: D3 · ≤ 2:00
- Poroporo: D3 · ≤ 1:45
- Parahi: D3 · ≤ 1:35
- Hiriwa: D3 · ≤ 1:28
- Kōura: D3 · ≤ 1:22
- Uenuku: D3 · ≤ 1:17
- Taniwha: D3 · ≤ 1:12

**Women**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 39.2s
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 1:15
- Kākāriki: D2 · ≤ 1:00
- Kahurangi: D3 · ≤ 2:15
- Poroporo: D3 · ≤ 2:00
- Parahi: D3 · ≤ 1:45
- Hiriwa: D3 · ≤ 1:40
- Kōura: D3 · ≤ 1:30
- Uenuku: D3 · ≤ 1:25
- Taniwha: D3 · ≤ 1:20

### 64. Ski Erg

*Scored by: difficulty level, then fastest time · 4 players have scored it*

> Drafted by hand. As Running. The median adult skis 1000m in about 5:00.

**Men**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:05
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 2:20
- Kākāriki: D2 · ≤ 2:05
- Kahurangi: D3 · ≤ 5:00
- Poroporo: D3 · ≤ 4:35
- Parahi: D3 · ≤ 4:15
- Hiriwa: D3 · ≤ 4:00
- Kōura: D3 · ≤ 3:45
- Uenuku: D3 · ≤ 3:35
- Taniwha: D3 · ≤ 3:25

**Women**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:15
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 2:35
- Kākāriki: D2 · ≤ 2:20
- Kahurangi: D3 · ≤ 5:35
- Poroporo: D3 · ≤ 5:10
- Parahi: D3 · ≤ 4:45
- Hiriwa: D3 · ≤ 4:30
- Kōura: D3 · ≤ 4:10
- Uenuku: D3 · ≤ 4:00
- Taniwha: D3 · ≤ 3:50

### 65. Row Erg

*Scored by: difficulty level, then fastest time · 4 players have scored it*

> Drafted by hand. As Running. The median adult rows 1000m in about 4:45.

**Men**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:05
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 2:15
- Kākāriki: D2 · ≤ 1:55
- Kahurangi: D3 · ≤ 4:45
- Poroporo: D3 · ≤ 4:15
- Parahi: D3 · ≤ 3:55
- Hiriwa: D3 · ≤ 3:40
- Kōura: D3 · ≤ 3:28
- Uenuku: D3 · ≤ 3:18
- Taniwha: D3 · ≤ 3:10

**Women**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:15
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 2:30
- Kākāriki: D2 · ≤ 2:10
- Kahurangi: D3 · ≤ 5:20
- Poroporo: D3 · ≤ 4:45
- Parahi: D3 · ≤ 4:25
- Hiriwa: D3 · ≤ 4:05
- Kōura: D3 · ≤ 3:55
- Uenuku: D3 · ≤ 3:40
- Taniwha: D3 · ≤ 3:35

### 66. Breath Hold

*Scored by: longest hold · 4 players have scored it*

> Drafted by hand. A static hold. The median untrained adult manages about a minute; three minutes takes years of practice.

**Everyone**

- Kiwikiwi: 10s
- Whero: 20s
- Karaka: 30s
- Kōwhai: 40s
- Kākāriki: 50s
- Kahurangi: 1:00
- Poroporo: 1:15
- Parahi: 1:30
- Hiriwa: 2:00
- Kōura: 2:30
- Uenuku: 3:00
- Taniwha: 4:00

### 67. Weighted Carry

*Scored by: difficulty level, then fastest time · 12 players have scored it*

> Approved in review.

**Everyone**

- Kiwikiwi: D1 · ≤ 4:00
- Whero: D1 · ≤ 2:30
- Karaka: D1 · ≤ 1:45
- Kōwhai: D2 · ≤ 4:00
- Kākāriki: D2 · ≤ 2:45
- Kahurangi: D2 · ≤ 2:00
- Poroporo: D3 · ≤ 4:00
- Parahi: D3 · ≤ 3:00
- Hiriwa: D3 · ≤ 2:15
- Kōura: D3 · ≤ 1:45
- Uenuku: D4 · ≤ 4:00
- Taniwha: D4 · ≤ 2:00

### 68. Animal Crawl

*Scored by: difficulty level, then fastest time · 0 players have scored it*

> Drafted by hand. Three colours on each of the easy crawls, two on the hard ones.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 30s
- Karaka: D1 · ≤ 20s
- Kōwhai: D2 · finish
- Kākāriki: D2 · ≤ 30s
- Kahurangi: D2 · ≤ 20s
- Poroporo: D3 · ≤ 45s
- Parahi: D3 · ≤ 30s
- Hiriwa: D4 · ≤ 40s
- Kōura: D4 · ≤ 25s
- Uenuku: D5 · ≤ 2:30
- Taniwha: D5 · ≤ 1:45

### 69. Bronco

*Scored by: difficulty level, then fastest time · 4 players have scored it*

> Drafted by hand. Taniwha is the full five-lap Bronco under five minutes, which is a professional rugby standard.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:15
- Karaka: D1 · ≤ 1:00
- Kōwhai: D2 · finish
- Kākāriki: D2 · ≤ 2:30
- Kahurangi: D2 · ≤ 2:05
- Poroporo: D3 · ≤ 4:00
- Parahi: D3 · ≤ 3:15
- Hiriwa: D4 · ≤ 5:15
- Kōura: D4 · ≤ 4:20
- Uenuku: D5 · ≤ 6:30
- Taniwha: D5 · ≤ 5:00

### 70. Scooting

*Scored by: difficulty level, then fastest time · 0 players have scored it*

> Drafted by hand. As Running, one ladder for everyone.

**Everyone**

- Kiwikiwi: D1 · finish
- Whero: D1 · ≤ 1:00
- Karaka: D2 · finish
- Kōwhai: D2 · ≤ 2:00
- Kākāriki: D2 · ≤ 1:45
- Kahurangi: D3 · ≤ 4:00
- Poroporo: D3 · ≤ 3:30
- Parahi: D3 · ≤ 3:10
- Hiriwa: D3 · ≤ 2:55
- Kōura: D3 · ≤ 2:45
- Uenuku: D3 · ≤ 2:35
- Taniwha: D3 · ≤ 2:25

### 71. Wheelbarrow Push

*Scored by: difficulty level, then fastest time · 0 players have scored it*

> Approved in review.

**Everyone**

- Kiwikiwi: D1 · ≤ 4:00
- Whero: D1 · ≤ 2:30
- Karaka: D1 · ≤ 1:45
- Kōwhai: D2 · ≤ 4:00
- Kākāriki: D2 · ≤ 2:45
- Kahurangi: D2 · ≤ 2:00
- Poroporo: D3 · ≤ 4:00
- Parahi: D3 · ≤ 3:00
- Hiriwa: D3 · ≤ 2:15
- Kōura: D3 · ≤ 1:45
- Uenuku: D4 · ≤ 4:00
- Taniwha: D4 · ≤ 2:00

### 72. Wheelbarrow Pull

*Scored by: difficulty level, then fastest time · 0 players have scored it*

> Approved in review.

**Everyone**

- Kiwikiwi: D1 · ≤ 4:00
- Whero: D1 · ≤ 2:30
- Karaka: D1 · ≤ 1:45
- Kōwhai: D2 · ≤ 4:00
- Kākāriki: D2 · ≤ 2:45
- Kahurangi: D2 · ≤ 2:00
- Poroporo: D3 · ≤ 4:00
- Parahi: D3 · ≤ 3:00
- Hiriwa: D3 · ≤ 2:15
- Kōura: D3 · ≤ 1:45
- Uenuku: D4 · ≤ 4:00
- Taniwha: D4 · ≤ 2:00

---

## 7. Flexibility

### 73. Rear Hand Clasp

*Scored by: difficulty level, then longest hold · 4 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 30s
- Taniwha: D7 · 30s

### 74. Bridge

*Scored by: difficulty level, then longest hold · 12 players have scored it*

> Approved in review.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 60s
- Karaka: D2 · 15s
- Kōwhai: D2 · 45s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 15s
- Taniwha: D6 · 60s

### 75. Forward Fold

*Scored by: difficulty level, then longest hold · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 30s
- Kōura: D6 · 30s
- Uenuku: D7 · 30s
- Taniwha: D8 · 30s

### 76. Needle Pose

*Scored by: difficulty level, then longest hold · 0 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 10s
- Taniwha: D6 · 30s

### 77. Forward Split

*Scored by: difficulty level, then longest hold · 5 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 10s
- Taniwha: D6 · 30s

### 78. Middle Split

*Scored by: difficulty level, then longest hold · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 30s
- Taniwha: D7 · 30s

### 79. Standing Split

*Scored by: difficulty level, then longest hold · 4 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 30s
- Taniwha: D7 · 30s

### 80. Foot Behind Head Pose

*Scored by: difficulty level, then longest hold · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 30s
- Taniwha: D7 · 30s

### 81. Shoulder Dislocate

*Scored by: narrowest hand width · 2 players have scored it*

> Drafted by hand. Hand width on the stick, narrower is better. A width depends on shoulder breadth, so a broad player is disadvantaged; say if it should be a ratio of shoulder width instead.

**Everyone**

- Kiwikiwi: any
- Whero: ≤ 120cm
- Karaka: ≤ 110cm
- Kōwhai: ≤ 100cm
- Kākāriki: ≤ 95cm
- Kahurangi: ≤ 90cm
- Poroporo: ≤ 85cm
- Parahi: ≤ 80cm
- Hiriwa: ≤ 75cm
- Kōura: ≤ 70cm
- Uenuku: ≤ 65cm
- Taniwha: ≤ 60cm

### 82. Pancake

*Scored by: difficulty level, then longest hold · 3 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s
- Poroporo: D4 · 10s
- Parahi: D4 · 30s
- Hiriwa: D5 · 10s
- Kōura: D5 · 30s
- Uenuku: D6 · 30s
- Taniwha: D7 · 30s

### 83. Side Bend

*Scored by: difficulty level, then longest hold · 0 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 20s
- Karaka: D1 · 45s
- Kōwhai: D2 · 10s
- Kākāriki: D2 · 20s
- Kahurangi: D2 · 45s
- Poroporo: D3 · 10s
- Parahi: D3 · 20s
- Hiriwa: D3 · 45s
- Kōura: D4 · 10s
- Uenuku: D4 · 20s
- Taniwha: D4 · 45s

### 84. Full Bound Twist

*Scored by: difficulty level, then longest hold · 2 players have scored it*

> Drafted by rule. Twelve colours spread over the rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 20s
- Karaka: D1 · 45s
- Kōwhai: D2 · 10s
- Kākāriki: D2 · 20s
- Kahurangi: D2 · 45s
- Poroporo: D3 · 10s
- Parahi: D3 · 20s
- Hiriwa: D3 · 45s
- Kōura: D4 · 10s
- Uenuku: D4 · 20s
- Taniwha: D4 · 45s

---

## 8. Body Awareness

### 85. Tae Kwon Do

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 8
- Karaka: D2 · 5
- Kōwhai: D2 · 8
- Kākāriki: D3 · 5
- Kahurangi: D3 · 8

*Poroporo and above: the head-to-head rating.*

### 86. Breakdancing

*Scored by: difficulty level, then longest hold · 0 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 30s
- Whero: D2 · 30s
- Karaka: D3 · 30s
- Kōwhai: D4 · 30s
- Kākāriki: D5 · 30s
- Kahurangi: D6 · 30s

*Poroporo and above: the head-to-head rating.*

### 87. Trampolining

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10
- Whero: D2 · 10
- Karaka: D3 · 10
- Kōwhai: D4 · 10
- Kākāriki: D5 · 10
- Kahurangi: D6 · 10

*Poroporo and above: the head-to-head rating.*

### 88. Jump Rope

*Scored by: difficulty level, then most reps · 12 players have scored it*

> Approved in review.

**Everyone**

- Kiwikiwi: D1 · 1
- Whero: D1 · 10
- Karaka: D2 · 10
- Kōwhai: D3 · 10
- Kākāriki: D4 · 10
- Kahurangi: D5 · 10

*Poroporo and above: the head-to-head rating.*

### 89. Wrestling

*Scored by: win, draw or loss against another player · 7 players have scored it*

**Rating only.** No fair solo drill exists, so every colour comes from the head-to-head rating.

### 90. Gymnastics

*Scored by: difficulty level, then most reps · 2 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10
- Whero: D2 · 10
- Karaka: D3 · 10
- Kōwhai: D4 · 10
- Kākāriki: D5 · 10
- Kahurangi: D6 · 10

*Poroporo and above: the head-to-head rating.*

### 91. Balance Ball

*Scored by: difficulty level, then longest hold · 5 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 30s
- Kōwhai: D3 · 30s
- Kākāriki: D4 · 30s
- Kahurangi: D5 · 30s

*Poroporo and above: the head-to-head rating.*

### 92. SKATE

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 10
- Kōwhai: D3 · 10
- Kākāriki: D4 · 10
- Kahurangi: D5 · 10

*Poroporo and above: the head-to-head rating.*

### 93. Fencing

*Scored by: difficulty level, then most reps · 6 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 8
- Karaka: D2 · 5
- Kōwhai: D2 · 8
- Kākāriki: D3 · 5
- Kahurangi: D3 · 8

*Poroporo and above: the head-to-head rating.*

### 94. Juggling

*Scored by: difficulty level, then longest hold · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 10s
- Kōwhai: D2 · 30s
- Kākāriki: D3 · 10s
- Kahurangi: D3 · 30s

*Poroporo and above: the head-to-head rating.*

### 95. Foot Juggling

*Scored by: difficulty level, then most reps · 0 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15

*Poroporo and above: the head-to-head rating.*

### 96. Slackline

*Scored by: difficulty level, then longest hold · 0 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 10s
- Whero: D1 · 30s
- Karaka: D2 · 30s
- Kōwhai: D3 · 30s
- Kākāriki: D4 · 30s
- Kahurangi: D5 · 30s

*Poroporo and above: the head-to-head rating.*

---

## 9. Coordination

### 97. Volleyball

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 98. Baseball

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 99. Teqball

*Scored by: difficulty level, then most reps · 5 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15

*Poroporo and above: the head-to-head rating.*

### 100. Tennis

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 101. Cricket

*Scored by: difficulty level, then most reps · 7 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 102. Badminton

*Scored by: difficulty level, then most reps · 15 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 103. Basketball

*Scored by: difficulty level, then most reps · 5 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 104. Football

*Scored by: difficulty level, then most reps · 5 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 105. Hockey

*Scored by: difficulty level, then most reps · 6 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 106. Squash

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 107. Lacrosse

*Scored by: difficulty level, then most reps · 0 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 108. Ultimate Frisbee

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

---

## 10. Aim & Precision

### 109. Netball

*Scored by: difficulty level, then most reps · 6 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 110. Bocce

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 111. Dodgeball

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 112. Carrom

*Scored by: difficulty level, then most reps · 16 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 10
- Karaka: D1 · 20
- Kōwhai: D2 · 5
- Kākāriki: D2 · 10
- Kahurangi: D2 · 20

*Poroporo and above: the head-to-head rating.*

### 113. Archery

*Scored by: difficulty level, then most reps · 5 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 114. Bowling

*Scored by: difficulty level, then most reps · 2 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 115. Darts

*Scored by: difficulty level, then most reps · 3 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

### 116. Disc Golf

*Scored by: difficulty level, then most reps · 7 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15

*Poroporo and above: the head-to-head rating.*

### 117. Golf

*Scored by: difficulty level, then most reps · 5 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15

*Poroporo and above: the head-to-head rating.*

### 118. Handball

*Scored by: difficulty level, then most reps · 0 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15

*Poroporo and above: the head-to-head rating.*

### 119. Table Tennis

*Scored by: difficulty level, then most reps · 0 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 5
- Kahurangi: D3 · 15

*Poroporo and above: the head-to-head rating.*

### 120. Kubb

*Scored by: difficulty level, then most reps · 4 players have scored it*

> Drafted by rule. Six drill colours spread over the drill rungs, extras on the lowest. Reach a rung, then own it.

**Everyone**

- Kiwikiwi: D1 · 5
- Whero: D1 · 15
- Karaka: D2 · 5
- Kōwhai: D2 · 15
- Kākāriki: D3 · 10
- Kahurangi: D4 · 10

*Poroporo and above: the head-to-head rating.*

---
