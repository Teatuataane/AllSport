...
| # | Domain | Events |
|---|--------|--------|
| 1 | Maximal Strength | 1A Press, Deadlift, Clean & Press, Pause Dips, Pause Chin Up, Pause Squat, Zercher Dead, Pause Bench, Turkish Get Up, Arthur Lift, Pause Row, Pause Front Squat |
| 2 | Calisthenics | 1 Leg Squat, Flag, Windshield Wipers, Toe Lift, Planche, Back Lever, Iron Cross, Front Lever, Chin Hang, Climbing, Handstand, Toe Balance |
| 3 | Power | Kelly Snatch, 1A Snatch, Triple Jump, Javelin, Shotput, Australian Football, Vertical Jump, Clean & Jerk, Snatch, Standing Broad Jump, High Jump, American Football |
| 4 | Speed | 100m Sprint, Tag, T-Race, 400m Race, Beach Flags, 50m Sprint, 200m Sprint, Touch Rugby, Football Dribble, Repeat High Jump, Rats & Rabbits, Speed Chess, Hockey Dribble |
| 5 | Anaerobic Endurance | Chinup Contest, Pushup Contest, Reverse Hyper, L-Sit Hold, Tibialis Curl, Headstand, Finger Push Up, GHD Situp, Leg Extension, Ab Rollout, Ham Curl, Sandbag to Shoulder, Wall Sit |
| 6 | Aerobic Endurance | Burpee Broad Jump, Running, Cycling, Ski Erg, Row Erg, Breath Hold, Weighted Carry, Duck Walk, Bronco, Walking, Backwards Walk, Scooting |
| 7 | Flexibility | Rear Hand Clasp, Bridge, Forward Fold, Needle Pose, Forward Split, Middle Split, Standing Split, Foot Behind Head, Shoulder Dislocate, Pancake, Side Bend, Full Bound Twist |
...
| 10 | Aim & Precision | Netball, Bocce, Dodgeball, Carrom, Archery, Bowling, Darts, Disc Golf, Golf, Airsoft, Handball, Table Tennis |
...
~/allsport/
  ...
  lib/
    ...
    eventData.ts                    # Single source of truth for all events (122) + difficulty+time encode/decode helpers (encodeDiffTime/decodeDiffTime/isTimedEffort, TIMED_EFFORT_SLUGS); DifficultyTier has optional `detail` (judge criteria)
  ...
  supabase/
    ...
    migrations/                       # All files renamed to unique 14-digit timestamps (YYYYMMDDHHMMSS) for CLI compatibility (July 2026)
      ...
      # NOTE: the three 2026071x migrations above are session-22 work NOT yet applied to prod — apply via `supabase db push` (or SQL Editor), then they auto-record as applied.
  public/
    logo.png
```
...
1. Apply the pending session-22 migrations. Migrations are now Supabase-CLI-managed: everything through `20260707000000_leaderboard_cleanup.sql` is applied to prod and baselined as applied (confirmed by Tāne, July 2026). The three session-22 migrations below are NOT yet applied — apply them with `supabase db push` (see `supabase/README.md`; it runs them in order and records them as applied). They can still be run in the SQL Editor instead, but do it ONE way only — never both, or the one-time transforms double-apply.
   - `20260713000000_fix_double_award.sql` (was `20260713`) — **run ONCE**: drops the orphaned `on_session_end` trigger (the ×2 games/points root cause), adds an atomic claim guard to `award_session_points`, and rebuilds 2026 rankings from `session_player_summary` (+ results fallback for pre-summary sessions). Includes a read-only diagnostic block at the top.
   - `20260713000001_breath_hold_duck_walk.sql` (was `20260713000001`) — **run ONCE, after the above**: flips existing Breath Hold raw_scores positive (hold mode) and re-encodes Duck Walk walk-tier rows to the new all-walk ladder (faster wins). One-time transforms with double-run guards.
   - `20260714000000_wellbeing_survey.sql` (was `20260714`) — creates `wellbeing_surveys` table + RLS + `get_wellbeing_report()` aggregate RPC (idempotent).
