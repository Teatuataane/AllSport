...
*Last updated: July 2026 (session 23 — /prs Personal Bests page: domains now collapsible (collapsed by default) with a new `DomainIcon` component (masked/tinted `/domain-icons/{slug}.png`, domain-number fallback), per-domain PB count, and 36px per-event `EventIcon`s revealed on expand (dimmed for no-result events); new `public/domain-icons/` folder awaiting Tāne's 10 Canva silhouette exports)*
