-- Fix award_session_points trigger — May 2026
-- Changes:
-- 1. Remove all session bonuses (attendance, PB, top performance, first session, streak, championship)
--    Bonus system was previously removed in design but not from the trigger — this caused 1st place
--    players to receive 140 pts instead of the correct 100 pts.
-- 2. Fix gap formula: remove GREATEST() floor on gap (gap should have NO minimum;
--    minimum 10 applies to awarded points only, not the gap itself).
-- 3. Total points = placement_points + effort_points ONLY (no bonus bucket).
...
BEGIN
  ...
  -- With multiple results per player per event, take the best (highest) raw_score.
